...
function main() {
	...
	if((count($validation_error)==0 )){ // checking the validation errors
		if(isset($_POST["Update"]) && isset($_GET["id"])){
			...
			$db_helper_obj->update_state($ins_state,$_GET["id"]);
			set_success_msg('State Updated Successfully');
			...
			$db_helper_obj->add_state($ins_state);
			set_success_msg('State Added Successfully');
			header("location: state.php");
			exit();
		}
	}else
		...
		if((count($validation_error)==0 )){
			if($data=$db_helper_obj->get_state_by_id($_GET["id"])){
				extract($data);
				$action="Update";
			}else
				$error="Invalid Data";
	...
	$state_list=$db_helper_obj->get_state_list($where,$where_arr,$sort_by); // getting all the users data
	$get_total_rows = count($state_list);
	
	if($get_total_rows > $item_per_page){
		$page_position = (($page_number-1) * $item_per_page);
		$state_list=$db_helper_obj->get_state_list($where,$where_arr," $sort_by LIMIT $page_position, $item_per_page");
...
<div class="container-fluid">
		...
		<?php } ?>
			<form name="state_form" method="post">
				<div class="form-row">
                    ...
                    <label for="name_id">State Code</label>
					<input name="name" type="text" class="form-control" id="name_id" placeholder="Code" value="<?php echo $name; ?>">
                    </div>
                </div>
				<div class="form-row">
				<div class="col-md-12">
    ...
    <div class="card mb-3">
        <div class="card-header">
          <i class="fa fa-address-book"></i>&nbsp;State List</div>
        <div class="card-body">
		  <div class="table-responsive">
			...
			<div class="row">
				...
				<?php echo get_success_alert();?>
					...
					<tbody>
					<?php if(count($state_list) > 0){ $cnt=0; foreach($state_list as $va=>$row){ $cnt++; ?>
						<tr>
						  <td><?php echo $cnt; ?></td>
						 ...
						 <td>
						  	<a href="state.php?id=<?php echo $row["id"]; ?>" title="Make it as Inactive">   <i class="fa fa-pencil"></i></a>
						 </td>
						</tr>
					<?php } }else{ ?>
                  <tr>
