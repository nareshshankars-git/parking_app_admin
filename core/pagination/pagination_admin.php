<?php function paginate($item_per_page, $total_records,  $page_url="")
{
    ...
    if($total_pages > 0 && $total_pages != 1 && $current_page <= $total_pages){ //verify total pages and current page number
        ...
        if($current_page > 1){
            ...
            $pagination .= '<li ><a href="?page=1'.$page_url.'" title="First">&laquo;</a></li>'; //first link
            $pagination .= '<li><a href="?page='.$previous_link.$page_url.'" title="Previous">&lt;</a></li>'; //previous link
                for($i = ($current_page-2); $i < $current_page; $i++){ //Create left-hand side links
                    if($i > 0){
                        $pagination .= '<li><a href="?page='.$i.$page_url.'">'.$i.'</a></li>';
                    }
                }   
        ...
        if($first_link){ //if current active page is first link
            $pagination .= '<li class=" active"><a>'.$current_page.'</a></li>'; //first
        }elseif($current_page == $total_pages){ //if it's the last active link
            $pagination .= '<li class=" active"><a>'.$current_page.'</a></li>';//last
        ...
        if($current_page < $total_pages){ 
                ...
                $pagination .= '<li><a href="?page='.$next.$page_url.'" title="Next">&gt;</a></li>'; //next link
                $pagination .= '<li class="last"><a href="?page='.$total_pages.$page_url.'" title="Last">&raquo;</a></li>'; //last link
        }
