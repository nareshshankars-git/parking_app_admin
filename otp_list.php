...
function main() {
	...
	$sort_by=get_sort_by();
	if($sort_by=="")
		$sort_by=" order by g.created_datetime desc"; // default sort by
	$where="1";
	$where_arr=array();
	$page_url_form="";
	...
	$transaction_list=$db_helper_obj->get_otp_list($where,$where_arr,$sort_by); // getting all the users data
	$get_total_rows = count($transaction_list);
	
	if($get_total_rows > $item_per_page){
		$page_position = (($page_number-1) * $item_per_page);
		$transaction_list=$db_helper_obj->get_otp_list($where,$where_arr," $sort_by LIMIT $page_position, $item_per_page");
				...
				<?php echo get_success_alert();?>
					...
					<?php if(count($transaction_list) > 0){ $cnt=0; foreach($transaction_list as $va=>$row){ $cnt++; ?>
						<tr>
						  ...
						  <td><?php if(isset($row["check_out"])) echo get_date_format($row["created_datetime"]); ?></td>
						</tr>
					<?php } }else{ ?>
                  <tr>
